...
typename EH_set<Key, N>::size_type EH_set<Key, N>::Bucket::append(const key_type &elem) {
    if (arrsz == N)
        return 0;
    elements[arrsz++] = elem;
    return 1;
...
// O(N) = O(1)
template <typename Key, size_t N>
typename EH_set<Key, N>::size_type EH_set<Key, N>::Bucket::find(const key_type &elem) const {
...
// O(N) = O(1)
template <typename Key, size_t N>
typename EH_set<Key, N>::size_type EH_set<Key, N>::Bucket::remove(const key_type &elem) {
    for (size_type i{0}; i < arrsz; ++i)
        if (key_equal{}(elem, elements[i])) {
            if (i != --arrsz)
...
template <typename Key, size_t N>
typename EH_set<Key, N>::size_type EH_set<Key, N>::Bucket::high_bit() const { return 1 << l; }
...
void EH_set<Key, N>::split_bucket(size_type hash) {
    ...
    // only the l+1 least significant bit must be checked, so rbitshift by l and test if bit is set
    // no temp copy needed, since we always check after newly added elements
    ...
    // assign every pointer that should point to new Bucket (2 * original offset)
...
// O(list size)
template <typename Key, size_t N>
EH_set<Key, N>::EH_set(std::initializer_list<key_type> ilist) : EH_set{std::begin(ilist), std::end(ilist)} {}

// calls list insert
// O(it range)
template <typename Key, size_t N>
template <typename InputIt> EH_set<Key, N>::EH_set(InputIt first, InputIt last) : EH_set{} {
    insert(first, last);
}
...
// O(other.nD)
template <typename Key, size_t N>
EH_set<Key, N>::EH_set(const EH_set &other): sz{other.sz}, d{other.d}, nD{other.nD}, buckets{new Bucket *[nD]} {
    for (size_t i{0}; i < nD; ++i) {
...
// O(nD + list size)
template <typename Key, size_t N>
EH_set<Key, N>& EH_set<Key, N>::operator=(std::initializer_list<key_type> ilist) {
    for (size_type i{0}; i < nD; ++i)
        buckets[i]->arrsz = 0;
    sz = 0;
    insert(ilist);
...
// O(1)
template <typename Key, size_t N>
typename EH_set<Key, N>::size_type EH_set<Key, N>::size() const { return sz; }
// O(1)
template <typename Key, size_t N>
bool EH_set<Key, N>::empty() const { return (sz == 0); }
...
// O(1)
template <typename Key, size_t N>
std::pair<typename EH_set<Key, N>::iterator, bool> EH_set<Key, N>::insert(const key_type &key) {
    size_type old_sz{sz}; 
...
// O(range size)
template <typename Key, size_t N>
template <typename InputIt> void EH_set<Key, N>::insert(InputIt first, InputIt last) {
...
// O(1)
template <typename Key, size_t N>
typename EH_set<Key, N>::size_type EH_set<Key, N>::count(const key_type &key) const {
...
// O(1)
template <typename Key, size_t N>
typename EH_set<Key, N>::iterator EH_set<Key, N>::find(const key_type &key) const {
...
// O(1)
template <typename Key, size_t N>
void EH_set<Key, N>::swap(EH_set &other) {
    using std::swap;
    swap(d, other.d);
    swap(nD, other.nD);
    swap(sz, other.sz);
...
private:
    ...
    // skip to the next, first ptr, that is the first occurence, to point to a non-empty Bucket
    void skip() {
...
public:
    explicit Iterator(size_type idx, size_type b, const EH_set *set) : idx{idx}, set{set}, b{b & (set->buckets[b]->high_bit() - 1)} {
        skip();
    }

    Iterator(const EH_set *set): idx{0}, set{set}, b{set->nD} {} // for end-iterator
